...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("🎯 Calibrating Heston for symbols: {:?}", symbols);
    println!();

    for symbol in &symbols {
        println!("🔍 Calibrating {}...", symbol);
        ...
        let (spot, all_options) = match load_options_from_json(&json_file) {
            Ok(data) => data,
            Err(e) => {
                println!("  ❌ Failed to load options: {}", e);
                continue;
            }
        };
        ...
        if liquid_options.is_empty() {
            println!("  ❌ No liquid options found!");
            continue;
        }
        ...
        println!("     κ (mean reversion) = {:.4}", result.params.kappa);
        println!("     θ (long-term vol)  = {:.4}", result.params.theta);
        println!("     σ (vol of vol)     = {:.4}", result.params.sigma);
        println!("     ρ (correlation)    = {:.4}", result.params.rho);
        println!("     v₀ (initial vol)   = {:.4}", result.params.v0);
        ...
        if let Ok(json_str) = serde_json::to_string_pretty(&params_data) {
            if std::fs::write(&filename, json_str).is_ok() {
                ...
                println!("  ❌ Failed to save parameters to {}", filename);
            }
        }
